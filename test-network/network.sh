...
## Call the script to deploy ERC20 Token chaincode to the channel
function deployTokenCC() {
  scripts/deployTokenCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE

  if [ $? -ne 0 ]; then
    fatalln "Deploying ERC20 Token chaincode failed"
  fi
}
...
elif [ "$MODE" == "deployTokenCC" ]; then
  infoln "deploying ERC20 token chaincode on channel '${CHANNEL_NAME}'"
else
  printHelp
  exit 1
fi
